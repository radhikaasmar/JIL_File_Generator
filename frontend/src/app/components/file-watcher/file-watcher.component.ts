...
export class FileWatcherComponent implements OnInit {
  ...
  private initializeFormControls() {
    ...
    this.environments.forEach(env => {
      if (!this.fileWatcherForm.contains(env.key)) {
        this.fileWatcherForm.addControl(env.key, new FormControl(false));
      }
    ...
    if (!this.fileWatcherForm.contains('conditions')) {
      this.fileWatcherForm.addControl('conditions', new FormArray([this.createCondition()]));
    }
    ...
    requiredControls.forEach(ctrl => {
      if (!this.fileWatcherForm.contains(ctrl)) {
  ...
  private setupFormValidation() {
    ...
    requiredFields.forEach(fieldName => {
      const control = this.fileWatcherForm.get(fieldName);
      if (control) {
        if (fieldName === 'csi') {
  ...
  clearForm() {
    if (confirm('Are you sure you want to clear all form data?')) {
      Object.keys(this.fileWatcherForm.controls).forEach(key => {
        const control = this.fileWatcherForm.get(key);
        if (control) {
          if (key === 'conditions') {
  ...
  get integratedJobValue(): string {
    if (!this.fileWatcherForm) return '';
    const fields = [
      'csi',
      'efforttype',
      'prodlob',
    ...
    const values = fields
      .map(f => this.fileWatcherForm.get(f)?.value || '')
      .filter(v => v !== '')
  ...
  get conditions(): FormArray {
    return this.fileWatcherForm.get('conditions') as FormArray;
  }
  ...
  onDayOfWeekChange(event: any) {
    ...
    const ctrl = this.fileWatcherForm.get('days_of_week');
    let days = ctrl?.value || [];
  ...
  isDaySelected(day: string): boolean {
    const days = this.fileWatcherForm.get('days_of_week')?.value || [];
    return days.includes(day);
  }

  isEnvironmentSelected(envKey: string): boolean {
    return this.fileWatcherForm.get(envKey)?.value === true;
  }
  ...
  validateForm(): boolean {
    ...
    const csiControl = this.fileWatcherForm.get('csi');
  ...
  onSubmit(): void {
    if (this.validateForm()) {
      console.log('Form submitted:', this.fileWatcherForm.value);
